...
import { Search, MapPin, Users, Heart, Check, Info, ZoomIn, ZoomOut, RotateCcw, ArrowLeft } from "lucide-react"
...
export default function EventSeatingPage() {
  ...
  useEffect(() => {
    const fetchEventData = async () => {
      try {
        ...
        setEvent(data)
        setAvailableTables(data.tables.filter((table: { seat_availabe: number }) => table.seat_availabe > 0))
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    setIsSearching(true)
    try {
      const response = await fetch(`/api/event/${eventId}/guests?q=${encodeURIComponent(searchQuery)}`)
  ...
  const selectGuest = (guest: Guest) => {
    ...
    // Reset zoom and pan when selecting a guest
    setZoomLevel(1)
  ...
  const handleRegister = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!registrationData.name || !registrationData.phone_number || !registrationData.table_no) return
    ...
    try {
      ...
      setAvailableTables(updatedTables.filter((table: { seat_availabe: number }) => table.seat_availabe > 0))
      setRegistrationComplete(true)

      // Select the newly registered guest
      setSelectedGuest({
      ...
      // Generate a personalized welcome message for new guests
      const welcomeMessages = [
  ...
  const toggleShowAllTables = () => {
    ...
    toast({
      title: showAllTables ? "Showing selected table only" : "Showing all tables",
      variant: "default",
    })
  }
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-gradient-to-b from-pink-50 to-purple-50">
        <div className="text-center">
          ...
          <p className="mt-4 text-lg font-medium text-primary">Loading event details...</p>
            ...
            <Card className="w-full border-2 border-pink-200 shadow-lg overflow-hidden">
              <CardHeader className="text-center bg-gradient-to-r from-pink-100 to-purple-100 rounded-t-lg pb-6">
                <div className="flex justify-center mb-2">
                  <GuestAvatar name={`${event.bride} ${event.groom}`} size="lg" className="border-2 border-pink-300" />
                </div>
              ...
              {selectedGuest ? (
                <CardContent className="pt-6 px-4 md:px-6">
                  <div className="text-center mb-6 bg-gradient-to-r from-pink-50 to-purple-50 p-4 rounded-lg border border-pink-200">
                    <div className="flex justify-center mb-3">
                      <GuestAvatar name={selectedGuest.name} imageUrl={selectedGuest.avatar_url} />
                    </div>
                    <h2 className="text-2xl font-medium mb-3 text-pink-600">{welcomeMessage}</h2>
                    <div className="flex items-center justify-center gap-2">
                      ...
                      <Tooltip>
                        ...
                        <TooltipContent>
                          <p>
                            With {selectedGuest.member_count} seat{selectedGuest.member_count !== 1 ? "s" : ""}
                          </p>
                        </TooltipContent>
                      </Tooltip>
                    </div>
                  ...
                  <div
                    ref={mapContainerRef}
                    className={`relative w-full ${isLargeScreen ? "aspect-[2/1]" : "aspect-[16/9]"} mb-4 border-2 border-pink-200 rounded-lg overflow-hidden bg-white shadow-md`}
                  ...
                  >
                    ...
                    >
                      ...
                      {event.tables.map((table) => {
                        if (showAllTables || table.table_no === selectedGuest.table_no) {
                          return (
                            <div
                              key={table.table_no}
                              className={`absolute ${table.table_no === selectedGuest.table_no ? "animate-bounce" : ""}`}
                              style={{
                            ...
                            >
                              <div
                                className={`flex flex-col items-center  -50%)",
                              }}
                            >
                              <div
                                className={\`flex flex-col items-center ${table.table_no === selectedGuest.table_no ? "scale-125" : ""}`}
                              >
                                <MapPin
                ...
                <>
                  <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                    ...
                    <TabsContent value="search" className="space-y-4 px-4 md:px-6">
                              ...
                              {searchResults.map((guest, index) => (
                                ...
                                >
                                  <div className="flex items-center gap-3">
                                    <GuestAvatar name={guest.name} imageUrl={guest.avatar_url} size="sm" />
                                    <div>
                                  ...
                                  <div className="flex items-center gap-1 bg-purple-100 px-2 py-1 rounded-full">
                                    ...
                                    <span className="text-sm font-medium text-purple-600">{guest.member_count}</span>
                                  </div>
                                </div>
                              ))}
                            </div>
                      ...
                      <CardContent className="pt-6 px-4 md:px-6">
                        {registrationComplete ? (
                          <div className="text-center py-6 space-y-4 bg-gradient-to-r from-pink-50 to-purple-50 rounded-lg border border-pink-200">
                            ...
                            <h3 className="font-medium text-xl text-purple-700">Registration Complete!</h3>
                            <p className="text-gray-700">
                          ...
                          <form onSubmit={handleRegister} className="space-y-4">
                            ...
                            <div className="space-y-2">
                              ...
                              <Input
                                ...
                                onChange={(e) =>
                                  setRegistrationData({
                                    ...registrationData,
                                    member_count: Number.parseInt(e.target.value) || 1,
                                  })
                                }
                                required
                                className="border-pink-200 focus-visible:ring-pink-400"
                            ...
                            <div className="space-y-2">
                              <Label className="text-gray-700">Select a Table</Label>
                                ...
                                {availableTables.map((table) => (
                                  <Tooltip key={table.table_no}>
                                    <TooltipTrigger asChild>
                                      <Button
                                        type="button"
                                        variant={registrationData.table_no === table.table_no ? "default" : "outline"}
                                        className={`h-auto py-2 flex flex-col ${
                                          registrationData.table_no === table.table_no
                                            ? "bg-gradient-to-r from-pink-500 to-purple-500 text-white hover:from-pink-600 hover:to-purple-600"
                                            : "border-pink-200 text-gray-700 hover:bg-pink-50"
                                        }`}
                                        onClick={() =>
                                          setRegistrationData({ ...registrationData, table_no: table.table_no })
                                        }
                                      >
                                        ...
                                        >
                                          {table.seat_availabe} seat{table.seat_availabe !== 1 ? "s" : ""} available
                                        </span>
                                      </Button>
                                    </TooltipTrigger>
                                    <TooltipContent>
                                      <p>{table.seat_assigned} guests already seated</p>
                                    </TooltipContent>
                                  </Tooltip>
                                ))}
                              </div>
                            ...
                            <Button
                              ...
                              disabled={isRegistering || !registrationData.table_no}
                            >
                              {isRegistering ? (
